...
class Trader:
    ...
    def compute_orders_ame(self, algo_bid: int, algo_ask: int, order_depth: OrderDepth):
        ...
        best_sell_price = outstanding_sell[0][0]
        best_buy_price = outstanding_buy[0][0]
    ...
    def compute_order_orc(self, algo_bid: int, algo_ask: int, order_depth: OrderDepth, observations: Observation):
        simple_obs = observations.plainValueObservations["ORCHIDS"]
        print(simple_obs)
        complex_obs = observations.conversionObservations["ORCHIDS"]
        print(complex_obs)
        bid_price = complex_obs.bid_price
        print(bid_price)
        ask_price = complex_obs.askPrice
        print(ask_price)
        transport_fees = complex_obs.transportFees
        print(transport_fees)
        export_tariff = complex_obs.exportTariff
        print(export_tariff)
        import_tariff = complex_obs.importTariff
        print(import_tariff)
        sunlight = complex_obs.sunlight
        print(sunlight)
        humidity = complex_obs.humidity
        print(humidity)
        return 
    ...
    def run(self, state: TradingState):        
        ...
        # ORCHIDS
        orc_orders = self.compute_order_orc(star_lb, star_ub, state.order_depths["ORCHIDS"], state.observations)
