...
class Trader:
    ...
    def compute_orders_orchid(self, algo_bid: int, algo_ask: int, state: TradingState, product: str):
        # standardised
        order_depth = state.order_depths[product]
        orders: list[Order] = []
        pos_lim = self.POS_LIMIT[product]
    ...
    def run(self, state: TradingState):        
        ...
        orc_orders = self.compute_orders_orchid(orc_lb, orc_ub, state, "ORCHIDS")
        result["ORCHIDS"] = orc_orders
            
        orchid_pos = self.position["ORCHIDS"]
        if orc_next_price > orc_mid_price and orchid_pos < 0:
