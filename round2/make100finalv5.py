...
class Trader:
    ...
    # #orchid export tariff cache
    # orch_expTariff_cache = []
    # orch_expTariff_terms = 10

    # def exportTariff_shorting_orchid(self, observation: Observation):
    #     if len(self.orch_expTariff_cache) != self.orch_expTariff_terms:
    #         return False

    #     orc_price = observation.askPrice
    #     expTariff = observation.exportTariff

    #     export_change = (self.orch_expTariff_cache[0]-expTariff)/self.orch_expTariff_cache[0]
    #     if export_change >= 0.1:
    #         if self.orch_expTariff_cache[0] > expTariff:
    #             return True
    ...
    def compute_orders_orchid(self, algo_bid: int, algo_ask: int, state: TradingState, product: str, observation: Observation):
        ...
        outstanding_sell = collections.OrderedDict(sorted(order_depth.sell_orders.items())) #reversed
        outstanding_buy = collections.OrderedDict(sorted(order_depth.buy_orders.items(), reverse=True)) #reversed
        
        best_sell_price = next(iter(outstanding_sell))
        best_buy_price = next(iter(outstanding_buy))
        # sell_vol, best_sell_price = self.values_extract(outstanding_sell)
        # buy_vol, best_buy_price = self.values_extract(outstanding_buy, 1)
        
        undercut_bid = best_buy_price + 0 #SET AT 0
        undercut_ask = best_sell_price - (best_sell_price-best_buy_price)/2 # 
        
        #initialising friction costs and aggression
        storage_price = 0.1
        bid_aggression = 0 # NOT TO CHANGE UNTIL MORE INFO IS OUT
        ask_aggression = 2.5 # 2.5 gives 67k
        ask_aggression = (best_sell_price - best_buy_price) * 0.25

        
        # # longing arbitrage, extremely conservative
        cur_pos = self.position[product]
        algo_bid = algo_bid - observation.exportTariff - observation.transportFees+ storage_price
        # market take outstanding sells
        for ask, vol in outstanding_sell.items():
            if (ask < algo_bid) or (cur_pos < 0 and (ask == algo_bid)) and cur_pos < pos_lim:
                order_amt = max(vol, pos_lim - cur_pos)
        ...
        if (cur_pos < pos_lim):
            order_amt = max(vol, pos_lim - cur_pos)
            # orders.append(Order(product, int(round(max(undercut_bid, algo_bid))), order_amt))
            orders.append(Order(product, int(round(algo_bid)), order_amt))


        
        # shorting arbitrage
        # algo_ask looks at the prices we are buying from the south at this current time - our minimal selling price
        algo_ask = algo_ask + observation.importTariff + observation.transportFees + ask_aggression
        # market take outstanding buys
        # for bid, vol in outstanding_buy.items():
        #     if (bid > algo_ask) or (cur_pos > 0 and (bid == algo_ask)) and cur_pos > -pos_lim:
        #         order_amt = max(-vol, -pos_lim - cur_pos)
        ...
        # if (cur_pos > -pos_lim) and self.exportTariff_shorting_orchid(observation):
        #     order_amt = -pos_lim-cur_pos
        #     orders.append(Order(product, int(math.floor(observation.askPrice + observation.importTariff + observation.transportFees)), order_amt))
        ...
        order_amt = -100
        #order_amt = -pos_lim-cur_pos
        # orders.append(Order(product, int(round(min(undercut_ask, algo_ask))), order_amt))
        orders.append(Order(product, int(math.floor(algo_ask)), order_amt))
    ...
    def run(self, state: TradingState):        
        ...
        # #we keep the last 20 prices of export tariffs
        # if len(self.orch_expTariff_cache) == self.orch_expTariff_terms:
        #     self.orch_expTariff_cache.pop(0)
        # self.orch_expTariff_cache.append(state.observations.conversionObservations["ORCHIDS"].exportTariff)
