...
class Trader:
    ...
    def lr_orchid(self, observation: Observation):
        ...
        next_price = intercept + orc_price*coef[2] + sun_coef*coef[1] + hum_coef*coef[0]
        return next_price
    ...
    def compute_orders_orchid(self, algo_bid: int, algo_ask: int, state: TradingState, product: str, observation: Observation):
        ...
        im_tariff = observation.importTariff
        ex_tariff = observation.exportTariff
        ...
        # market take outstanding buys
        for bid, vol in outstanding_buy.items():
            if self.exportTariff_shorting_orchid(observation) or (bid > algo_ask) or (cur_pos > 0 and (bid == algo_ask)) and (cur_pos > -pos_lim):
                order_amt = max(-vol, -pos_lim - cur_pos)
                cur_pos += order_amt
                orders.append(Order(product, bid, order_amt))
    ...
    def run(self, state: TradingState):        
        ...
        # ORCHIDS
        orc_next_price = self.lr_orchid(state.observations.conversionObservations["ORCHIDS"])
        orc_lb = orc_next_price - 1.6 # replace with logic for orc_next_bid later
        orc_ub = orc_next_price + 1.6
        ...
        if orc_next_price > state.observations.conversionObservations["ORCHIDS"].askPrice and orchid_pos < 0:
            conversions = -orchid_pos
        
        # if we are at local maxima, we convert the long position
        storage_price = 0.1
        if state.observations.conversionObservations["ORCHIDS"].bidPrice - orc_next_price <= storage_price and orchid_pos > 0:
            conversions = orchid_pos
