...
class Trader:
    ...
    def compute_orders_ame(self, algo_bid: int, algo_ask: int, order_depth: OrderDepth):
        ...
        outstanding_sell = collections.OrderedDict(sorted(order_depth.sell_orders.items())) #added reverse=True
        outstanding_buy = collections.OrderedDict(sorted(order_depth.buy_orders.items() , reverse=True )) #removed reverse=True
        ...
        undercut_b = best_buy_price # i minus 1 extra 
        undercut_s = best_sell_price # i minus 1 extra
        ...
        if (cur_pos < pos_lim):
            ...
            # else:
            orders.append(Order(product, min(undercut_b, algo_bid-1), vol_tobuy)) # i minus 1 extra
            cur_pos += vol_tobuy
        ...
        if (cur_pos > -pos_lim):
            ...
            #     orders.append(Order(product, max(undercut_s+1, algo_ask+1), vol_tosell))
            # else:
            orders.append(Order(product, max(undercut_s, algo_ask+1), vol_tosell))
    ...
    def run(self, state: TradingState):        
        ...
        s_vol, best_sell_star = self.values_extract(collections.OrderedDict(sorted(state.order_depths["STARFRUIT"].sell_orders.items()))) # added , reverse=True
        b_vol, best_buy_star = self.values_extract(collections.OrderedDict(sorted(state.order_depths["STARFRUIT"].buy_orders.items(), reverse=True)), 1) #removed , reverse=True
