...
class Trader:
    ...
    #volatility cache for AS
    volatilities = {'AMETHYSTS': 0.01, 'STARFRUIT': 0}
    ...
    def AS_calc(self, product, gamma, mid_price):
        q = self.position[product]
        omega = self.POS_LIMIT[product]
        s = mid_price
        sigma = self.volatilities[product]
        ask = s + (1/gamma) * np.log(1 + ((1- 2*q) * (gamma ** 2) * (sigma ** 2)) / (2 * omega - (gamma**2) * (q*2) * (sigma**2)) )
        bid = s + (1/gamma) * np.log(1 + ((-1- 2*q) * (gamma ** 2) * (sigma ** 2)) / (2 * omega - (gamma**2) * (q*2) * (sigma**2)) )
        return int(round(ask)), int(round(bid))
    ...
    def compute_orders_ame(self, algo_bid: int, algo_ask: int, order_depth: OrderDepth):
        ...
        # AS attempt
        mid_price = 10000
        ask, bid = self.AS_calc('AMETHYSTS', 0.15, mid_price)
        
        # buys
        amt_to_buy = pos_lim - cur_pos
        orders.append(Order(product, ask, amt_to_buy))
        
        # sells
        amt_to_sell = -pos_lim - cur_pos
        orders.append(Order(product, bid, amt_to_sell))
