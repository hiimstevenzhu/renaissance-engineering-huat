...
class Trader:
    ...
    # Make a copy of the order_dict to avoid modifying the original
        temp_dict = order_dict.copy()

        # # Ensure the dictionary has at least two items
        # if len(temp_dict) >= 2:
        #     # Pop the last item (which we don't need)
        #     temp_dict.popitem()
        #     # Pop the second-to-last item (now the last) to get its key (price)
        #     best_val, _ = temp_dict.popitem()
    ...
    def compute_orders_ame(self, algo_bid: int, algo_ask: int, order_depth: OrderDepth):
        ...
        outstanding_sell = collections.OrderedDict(sorted(order_depth.sell_orders.items(), reverse=True)) #added reverse=True
        outstanding_buy = collections.OrderedDict(sorted(order_depth.buy_orders.items() )) #removed reverse=True
        ...
        if (cur_pos < pos_lim):
            ...
            if cur_pos < 0:
                ...
                orders.append(Order(product, min(undercut_b, algo_bid-1), vol_tobuy)) # this line is different - we dont further undercut our buys # i minus 1 extra from from undercut_b
            elif cur_pos > 15:
                orders.append(Order(product, min(undercut_b-2, algo_bid-1), vol_tobuy)) # i minus 1 extra from from undercut_b
            else:
                orders.append(Order(product, min(undercut_b-1, algo_bid-1), vol_tobuy)) # i minus 1 extra from from undercut_b
            cur_pos += vol_tobuy
        ...
        if (cur_pos > -pos_lim):
            ...
            if cur_pos > 0:
                orders.append(Order(product, max(undercut_s, algo_ask+1), vol_tosell)) # i plus 1 extra from from undercut_s
            elif cur_pos < -15:
                orders.append(Order(product, max(undercut_s+2, algo_ask+1), vol_tosell))# i plus 1 extra from from undercut_s
            else:
                orders.append(Order(product, max(undercut_s+1, algo_ask+1), vol_tosell))     # i plus 1 extra from from undercut_s
    ...
    def compute_orders_star(self, algo_bid: int, algo_ask: int, order_depth: OrderDepth):
        ...
        for ask, vol in outstanding_sell.items():
            if (ask < algo_bid) or ((cur_pos < 0  and (ask == algo_bid))) and cur_pos < pos_lim: # i replaced algo_bid w undercut_b
                order_amt = min(-vol, pos_lim - cur_pos)
                cur_pos += order_amt
                orders.append(Order(product, ask, order_amt))
        
        bid_price = min(undercut_b, algo_bid-1) #algo_bid#undercut_b #min(undercut_b, algo_bid-1) #min(undercut_b, algo_bid-1)
        ask_price = max(undercut_s, algo_ask+1) #algo_ask#undercut_s#max(undercut_s, algo_ask+1)#max(undercut_s, algo_ask+1)
        
        if cur_pos < pos_lim:
            order_amt = pos_lim - cur_pos
            # if cur_pos < 0:
            #     orders.append(Order(product, min(undercut_b+1, algo_bid-1), order_amt))
            # elif cur_pos >15:
            #     orders.append(Order(product, min(undercut_b-1, algo_bid-1), order_amt))
            # else:
        ...
        for bid, vol in outstanding_buy.items():
            if (bid > algo_ask) or (cur_pos > 0 and (bid == algo_ask)) and cur_pos > -pos_lim: # i replaced algo_ask w undercut_s
                order_amt = max(-vol, -pos_lim - cur_pos)
                cur_pos += order_amt
                orders.append(Order(product, bid, order_amt))
        ...
        if cur_pos > -pos_lim:
            order_amt = -pos_lim-cur_pos
            # if cur_pos > 0:
            #     orders.append(Order(product, max(undercut_s-1, algo_ask+1), order_amt))
            # elif cur_pos < -15:
            #     orders.append(Order(product, max(undercut_s+1, algo_ask+1), order_amt))
            # else:
    ...
    def run(self, state: TradingState):        
        ...
        s_vol, best_sell_star = self.values_extract(collections.OrderedDict(sorted(state.order_depths["STARFRUIT"].sell_orders.items(), reverse=True))) # added , reverse=True
        b_vol, best_buy_star = self.values_extract(collections.OrderedDict(sorted(state.order_depths["STARFRUIT"].buy_orders.items())), 1) #removed , reverse=True
        self.starfruit_cache.append((best_buy_star+best_sell_star) / 2)
        star_lb = -INF
        star_ub = INF
        if len(self.starfruit_cache) == self.starfruit_terms:
            star_next_price = self.ar_starfruit()
            star_lb = star_next_price-1#int(star_next_price*0.99) #star_next_price-1
            star_ub = star_next_price+1#math.ceil(star_next_price*1.01) #star_next_price+1
