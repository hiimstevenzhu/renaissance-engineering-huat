    ...
    def compute_orders_coconuts(self, order_depth: OrderDepth, timestamp: int, state: TradingState):
        if 'COCONUT' in state.market_trades:
                for i in state.market_trades['COCONUT']:
                    if i.buyer == 'Raj':
                        self.raj_coconut = 'LONG' 
                    if i.seller == 'Raj':
                        self.raj_coconut = 'SHORT'
        premium = 637.63
        coc = 'COCONUT'
        cou = 'COCONUT_COUPON'
        orders = {coc: [], cou: []}
        ...
        if self.raj_coconut == 'SHORT':
            coco_vol = self.POS_LIMIT[coc] + positions[coc]
            orders[coc].append(Order(coc, best_sell[coc]-1, -coco_vol))

        elif self.raj_coconut == 'LONG':
            coco_vol = self.POS_LIMIT[coc] - positions[coc]
            orders[coc].append(Order(coc, best_buy[coc]+1, coco_vol))
