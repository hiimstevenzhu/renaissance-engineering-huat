...
class Trader:
    ...
    def compute_orders_coconuts(self, order_depth: OrderDepth, timestamp: int):
        ...
        if len(self.coupon_cache) > 2:
            price = int(math.floor(self.coupon_cache[-1]))
            if self.coupon_cache[-2] > self.coupon_cache[-1]: #want to buy
                orders[cou].append(Order(cou, price, 300- positions[cou]))
        ...
        if len(self.coconut_cache) > 2:
            price = int(math.floor(self.coconut_cache[-1]))
            if self.coconut_cache[-2] > self.coconut_cache[-1]:
                orders[coc].append(Order(coc, price, 300- positions[coc]))
