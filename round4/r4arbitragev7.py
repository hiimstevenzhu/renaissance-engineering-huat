...
class Trader:
    ...
    def compute_orders_coconuts(self, order_depth: OrderDepth):
        premium = 637.63
        coc = 'COCONUT'
        cou = 'COCONUT_COUPON'
        orders = {coc: [], cou: []}
        ...
        self.coconut_cache.append((best_buy[coc]+best_sell[coc]) / 2)
            
        if len(self.coupon_cache) == self.coupon_cache:
                self.coupon_cache.pop(0)
        self.coupon_cache.append((best_buy[cou]+best_sell[cou]) / 2)


        coc_diff = -300 - positions[coc]

        coc_diff = 300 - positions[coc]


        if self.coupon_cache[-2] > self.coupon_cache[-1]: #want to buy
            # price = self.coupon_cache[-1]
            orders[cou].append(Order(cou, best_sell[cou], 300- positions[cou]))
        elif self.coupon_cache[-2] < self.coupon_cache[-1]:
            orders[cou].append(Order(cou, best_buy[cou], -300- positions[cou]))
        
        if self.coconut_cache[-2] > self.coconut_cache[-1]:
            orders[coc].append(Order(coc, best_sell[coc], 300- positions[coc]))
        elif self.coconut_cache[-2] < self.coconut_cache[-1]:
            orders[coc].append(Order(coc, best_buy[coc], -300- positions[coc]))

        # trade_at = 0

        
        # derivative_price = mid_price[coc] - mid_price[cou] - 10000 #- premium
        # if self.prev_coc_derivatives_price == 0:
        #     self.prev_coc_derivatives_price = derivative_price
        # dev = derivative_price - self.prev_coc_derivatives_price
        ...
        #     coc_diff = -300 - positions[coc]
        #     orders[coc].append(Order(coc, best_buy[coc]-coc_cutting, coc_diff))
        #     orders[cou].append(Order(cou, worst_sell[cou]+cou_cutting, -coc_diff))
        #     # hold our zscore for exit
        # if dev <= -trade_at:
        #     # undervalued, long gb short rest
        #     coc_diff = 300 - positions[coc]
        #     orders[coc].append(Order(coc, best_sell[coc]+coc_cutting, coc_diff))
        #     orders[cou].append(Order(cou, worst_buy[cou]-cou_cutting, -coc_diff))
        #     # hold our zscore for exit
    ...
    def run(self, state: TradingState):        
        ...
        if 'COCONUTS' in state.order_depths.keys():
            coconut_orders = self.compute_orders_coconuts(state.order_depths)
            result['COCONUT'] = coconut_orders['COCONUT']
            result['COCONUT_COUPOM'] = coconut_orders['COCONUT_COUPON']
